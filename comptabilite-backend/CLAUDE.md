...
- **Invoice AI extraction**: `POST /api/purchase-invoices/extract` calls Claude API via RestTemplate (no SDK). Requires `ANTHROPIC_API_KEY` env var (set in `application-dev.yml` with fallback, `application-prod.yml` without fallback for fast-fail). Base64 data URL sent in body; PDFs use `"type":"document"` block, images use `"type":"image"`. Partial extraction (null fields) returns 200. 30s read timeout configured in AppConfig RestTemplate bean. Parse failures logged at WARN level.
